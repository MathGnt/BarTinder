...
struct CocktailCreationTests {
    ...
    func ingredientsThrowingFields(invalidMeasures: String) throws {
        let newCocktail = Cocktail(ingredients: [
            Ingredient(name: "tonic water", measure: invalidMeasures, unit: .cl),
            Ingredient(name: "gin", measure: "12", unit: .cl),
            Ingredient(name: "lime", measure: "1", unit: .wedge)
        ]
        )
        
        let noIngredients = Cocktail(name: "New Cocktail", ingredients: [], cocktailDescription: "A new cocktail")
        
        #expect(throws: CreationErrors.emptyMeasuresFields) {
            try ingredientModel.checkForIngredients(newCocktail.ingredients)
            try ingredientModel.checkForIngredients(noIngredients.ingredients)
        }
    }
